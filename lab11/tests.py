...
class TestHelper:
    ...
    @staticmethod
    def fail_non_empty_node(object_storage: ObjectStorage):
        non_empty_data_nodes = [node for node in object_storage.data_nodes if node.get_disk_usage() > 0 and node.is_healthy]
        random.choice(non_empty_data_nodes).is_healthy = False
